...
def get3D (real_kpts,mirror_kpts):
    ...
    for i in range(0,len(real_kpts)):
        if mirror_kpts[i][2]==0.0:
            real_projected[i]=[-1,-1]  #kpts w/ confidence of 0 will be negative so it's not drawn on
            points_3D[i] = [np.nan]
        if real_kpts[i][2]==0.0:
            real_kpts[i][0:2] = [-1,-1]
            real_projected[i]=real_kpts[i][0:2]  #kpts w/ confidence of 0 will be negative so it's not drawn on
            points_3D[i] = [np.nan]
    real_projected = np.append(real_projected,real[:,0:2],axis=1)
