    ...
    for r in joints:
        ...
        for i in r:
            if(start==0 and pts_3d[i][0]>=0):
                start_pt_3d = (int(pts_3d[i][0]),int(pts_3d[i][1]))
                start_pt_2d = (int(pts_3d[i][2]),int(pts_3d[i][3]))
                start = 1
                
            elif(pts_3d[i][0]>=0):
                end_pt_3d = (int(pts_3d[i][0]),int(pts_3d[i][1]))
                end_pt_2d = (int(pts_3d[i][2]),int(pts_3d[i][3]))
                img = cv2.line(img, start_pt_3d, end_pt_3d, color=(200, 200, 0,180), thickness=9)  
                img = cv2.circle(img, start_pt_3d, radius=8, color=(255, 255, 255,180), thickness=-1)

                img = cv2.line(img, start_pt_2d, end_pt_2d, color=(200, 0, 200,180), thickness=9)  
                img = cv2.circle(img, start_pt_2d, radius=8, color=(0, 0, 0,180), thickness=-1)
                start_pt_3d = end_pt_3d
                start_pt_2d = end_pt_2d
        if(start==1):   
            img = cv2.circle(img, start_pt_3d, radius=8, color=(255, 255, 255,180), thickness=-1)  #draw last joint 
            img = cv2.circle(img, start_pt_2d, radius=8, color=(0, 0, 0,180), thickness=-1)  #draw last joint 
