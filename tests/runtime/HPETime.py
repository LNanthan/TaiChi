...
#time to run openpose, measure spine length, match mirror kpts w/ real, get 3D kpts & overall time
f = open('tests/runtime/timeData/HPE', 'w+')
writer = csv.writer(f)
header = ['run openpose', 'measure spine', 'match kpts', 'get 3D','total time per frame']
...
#match the mirror joints to the real joints
...
while True:
    try:
        ...
        row.append(time.time()-start_task)
        start_task = time.time()  
